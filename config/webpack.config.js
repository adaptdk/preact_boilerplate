...
switch (process.argv[2]) {
  ...
  case 'start':
    rewireModule('react-scripts/scripts/start.js', loadCustomizer('../webpack.dev'));
    break;
    // The "build" script is run to produce a production bundle
  case 'build':
    rewireModule('react-scripts/scripts/build.js', loadCustomizer('../webpack.prod'));
    break;
    // The "test" script runs all the tests with Jest
  case 'test':
    // Load customizations from the config-ov errides.testing file.
